...
// PERBAIKAN: Memastikan format IPv4 menghasilkan 4 oktet lengkap (X.X.X.X)
const dip = () => [
...
async function pro(id) {
  const now = Date.now()
  const o = `SHINKA.3381-${String(now).slice(-4)}-${String(now).slice(-4)}-${crypto.randomBytes(2).toString('hex')}`
  
  // PERBAIKAN: Menambahkan 'purchaseState: 0' agar status transaksi terbaca sukses oleh fungsi verifikasi Alight Creative
  const b = {
    data: {
  ...
  try {
    ...
    return { ok: true, order: o, r: r.data, activatedAt: now }
  } catch (e) { return { ok: false, why: bad(e) } }
}
